...
<div class="my-3 p-3 bg-body rounded shadow-sm">
    <div class="d-flex justify-content-between border-bottom py-2">
        <h3 class="pb-2 mb-0">Data Kategori Asset</h3>
            ...
            <tbody>
                <?php if (!empty($kategori)) : ?>
                    <?php foreach ($kategori as $key => $row) : ?>
                        <tr>
                            <td><?= $key + 1 ?></td>
                            <td><?= $row['kode_kategori'] ?> - <?= $row['nama_kategori'] ?></td>
                            <td><?= esc($row['nama_kategori']) ?></td>
                            <td>
                                <a href="/kategori/edit/<?= $row['id'] ?>" class="btn btn-warning btn-sm">Edit</a>
                                <form action="/kategori/delete/<?= $row['id'] ?>" method="post" class="d-inline form-delete">
                                    <?= csrf_field() ?>
